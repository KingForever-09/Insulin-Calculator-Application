...
const FOOD_DB = [
  ...
  {name:'ก๋วยเตี๋ยวน้ำ (1 ชาม)', carbs:45},
...
function renderFoodList(){
  ...
  const list = q ? FOOD_DB.filter(f=>f.name.includes(q)) : FOOD_DB;
  const el = document.getElementById('foodResults');
  if(!list.length){ el.innerHTML = '<div class="empty">ไม่พบอาหารที่ค้นหา</div>'; return; }
